...
public class TupleDesc implements Serializable {
    ...
    public int fieldNameToIndex(String name) throws NoSuchElementException {
        ...
        throw new NoSuchElementException("no such filed");
    }
    ...
    public static TupleDesc merge(TupleDesc td1, TupleDesc td2) {
        // some code goes here
        int n = td1.tdItemList.size() + td2.tdItemList.size();
        Type[] t1 = new Type[n];
        String[] filedName = new String[n];
        for (int i = 0; i < td1.tdItemList.size(); i++) {
            t1[i] = td1.tdItemList.get(i).fieldType;
        }

        for (int i = 0; i < td2.tdItemList.size(); i++) {
            t1[i + td1.tdItemList.size()] = td2.tdItemList.get(i).fieldType;
        }

        for (int i = 0; i < td1.tdItemList.size(); i++) {
            filedName[i] = td1.tdItemList.get(i).fieldName;
        }

        for (int i = 0; i < td2.tdItemList.size(); i++) {
            filedName[i + td1.tdItemList.size()] = td2.tdItemList.get(i).fieldName;
        }
        return new TupleDesc(t1, filedName);
    }
    ...
    public boolean equals(Object o) {
        ...
        if (o instanceof TupleDesc t) {
            if (t.tdItemList.size() != tdItemList.size()) {
                return false;
            }
            int size = tdItemList.size();
    ...
    public String toString() {
        // some code goes here
        return tdItemList.stream().map(v ->
